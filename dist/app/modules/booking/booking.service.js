...
function createBookingIntoDb(user, payload, next) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if ((0, checkValidDate_1.default)(payload.date)) {
            if (timeRegex.test(payload.startTime)) {
                try {
                    ...
                    if (!fullCarObj || fullCarObj.isDeleted) {
                        return { success: false, statusCode: http_status_1.default.BAD_REQUEST, message: 'The car ID you provided is incorrect or the car is past way', data: [] };
                    }
                    if (fullCarObj.status === 'unavailable') {
                        return { success: false, statusCode: http_status_1.default.BAD_REQUEST, message: 'Car is not available right now!', data: [] };
                    }
                    ;
                    const userObj = yield user_model_1.default.findOne({ email: user.user }).session(session);
                    if (!userObj) {
                        return { success: false, statusCode: http_status_1.default.BAD_REQUEST, message: 'Booking Unsuccessful', data: [] };
                    }
                    ;
                    ...
                    ;
                    const dataForServer = { car: fullCarObj._id, date: payload.date, startTime: payload.startTime, user: userObj === null || userObj === void 0 ? void 0 : userObj._id };
                    ...
                    return { success: true, statusCode: http_status_1.default.OK, message: 'Car booked successfully', data: populatedBooking };
                }
                catch (error) {
            ...
            return { success: false, statusCode: http_status_1.default.BAD_REQUEST, message: 'Invalid Date.', data: [] };
        }
    });
}
; //end
function getUserSpecificBookingsFromDb(user, next) {
    return __awaiter(this, void 0, void 0, function* () {
        const session = yield mongoose_1.default.startSession();
        try {
            session.startTransaction();
            ...
            if (!userObj) {
                return { success: false, statusCode: http_status_1.default.BAD_REQUEST, message: 'Failed to fetch booking', data: [] };
            }
            ;
            const bookings = yield booking_model_1.default.find({ user: userObj._id }).session(session).populate('car user');
            if (!bookings.length) {
                return { success: false, statusCode: http_status_1.default.BAD_REQUEST, message: 'Failed to fetch booking', data: [] };
            }
            ;
            yield session.commitTransaction();
            yield session.endSession();
            return { success: true, statusCode: http_status_1.default.OK, message: 'My Bookings retrieved successfully', data: bookings };
        }
        catch (error) {
            yield session.abortTransaction();
            yield session.endSession();
        ...
        try {
            if (query.carId && !query.date) {
                ...
                if ((0, checkValidDate_1.default)(query.date)) {
                    ...
                    return { success: false, statusCode: http_status_1.default.BAD_REQUEST, message: 'Date Not Valid', data: [] };
                }
                ;
            }
            else if (query.date && query.carId) {
            ...
            ;
            if (!bookings.length) {
                return { success: false, statusCode: http_status_1.default.NOT_FOUND, message: 'Not Found', data: [] };
            }
            return { success: true, statusCode: http_status_1.default.OK, message: 'Bookings retrieved successfully', data: bookings };
        }
        catch (error) {
            next(error);
        }
...
exports.BookingServices = { createBookingIntoDb, getUserSpecificBookingsFromDb, getAllBookingsFromDb };
